...
# awww stores cache in a versioned subdir: ~/.cache/awww/<version>/<monitor>
cache_dir=""
for _d in "$HOME/.cache/awww"/*/; do
  [[ -d "$_d" ]] && cache_dir="$_d"
done
read_cached_wallpaper() {
  local cache_file="$1"
  if [[ -f "$cache_file" ]]; then
    # awww cache format: "crop Lanczos3 /path/to/image.jpg" — path is last field
    awk 'NF {print $NF; exit}' "$cache_file"
  fi
}
